...
- **Patchright requires real Google Chrome** (`channel: "chrome"`), installed in the Docker image via `npx patchright install --with-deps chrome`. It runs **headful** for full stealth, so the container needs an X display — `entrypoint.sh` starts `Xvfb :99` and exports `DISPLAY` before running the test (and before writing the cron env, so the daily job inherits it). `xvfb-run` is NOT used because the image lacks `xauth`; a plain background `Xvfb` is started instead.
- Local browser runs on WSL Ubuntu 20.04 are unreliable (old system libs) — run the browser inside Docker. TypeScript/ESLint/Prettier still work locally.
